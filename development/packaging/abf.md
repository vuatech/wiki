...
## Post Publish
Once a package is published to the ABF it should become available in a few minutes. If the package is a library that has a major change or has a package that is dependent on it's specific version it may be necessary to increase the ```Release:``` tag by 1 and rebuilding the package. Dependent packages can be determined by using ```dnf repoquery --whatdepends %{packagename}```. Below is an example of lib64hyprutils.
```
...
```
A way to test if the dependent packages need to be rebuilt is to install all applications from the ```dnf repoquery --whatdepends %{packagename}``` and install the non-debug RPM file generated by the build process after. The DNF command will return the something similar to the following below. If the RPM installs without issue then the dependent packages will work without rebuilds.
```
